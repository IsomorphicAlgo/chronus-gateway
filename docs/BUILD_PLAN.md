...
**Showcase & demos (orthogonal to M/CV):** one-command demos, Open MCT or SPA dashboard, replay, and optional public fixtures — **[`SHOWCASE_PLAN.md`](SHOWCASE_PLAN.md)** with gates **S0–S4** and manual acceptance **[`Demo_Test.md`](Demo_Test.md)**; checklist hooks in **[`../TEST_PLAN.md`](../TEST_PLAN.md#showcase-tracks)**. **S1** (Docker + **[`DEMO.md`](DEMO.md)**) deliverables are implemented — **Gate S-1** pending.
