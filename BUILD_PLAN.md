...
- [x] `main` wires the local M1–M4 logging pipeline: UDP ingest → CCSDS parse → throttled tracking
      state → co-validation. Production configuration files/env wiring, SDR RF metadata, and
      WebSocket fan-out remain M5+ work.
