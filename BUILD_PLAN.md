...
- [x] `main` wires the current demonstration pipeline: ingest → CCSDS parse → tracking provider
      → co-validation → structured logs. Runtime config is still default/in-code until a future
      configuration slice.
